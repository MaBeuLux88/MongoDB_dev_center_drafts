...
Some of these limitations hit us in a recent project in which we developed a web shop. The rather obvious requirement
...
and that Lucene is a mighty tool for text search, but we were actually surprised how well it supports our filtering use
case.
...
your tests against, or you use some kind of test containers to set the stage for your tests. Although Atlas Search
queries seamlessly integrate in MongoDB aggregation pipelines on Atlas, standard MongoDB cannot process this type of
aggregation stage.
...
cumbersome to set up this local cluster and to start/stop it every time you want to run your tests. This has to be done
...
which does not need a manual setup for these containers and which enables automatic start and shutdown. Turns out there
...
Atlas CLI uses two docker images to create an environment which enables testing Atlas Search queries locally:
...
docker–compose.yml needed for this blog post. The naming of the containers is based on the naming convention of the
...
environment we defined before. After the containers have been started, the script builds a connection string in order to
be able to connect to the cluster using a MongoClient (mind the `directConnection=true` parameter!). The mongoClient
connects to the cluster and issues an admin command in order to initialize the replica set. Since this command takes
...
and save a reference to the docker environment in order to tear it down after the tests have run.
...
some clean-up logic that runs after each test. Due to the fact that the Atlas Search index is located on another
...
- We create test data for each test suite. Inserting test data once per test suite using a beforeAll() method brings
  down
  the time we have to wait for the `mongot` container to process the updates as. The disadvantage of this approach is
  that
...
- We create global test data for all test suites. Using the global setup method from the last section, we could also
  insert data into the database before creating the index. When the initial index creation has been completed, we would
  be
  ready to run our tests without waiting for some events from the change stream being processed. But also in this
  scenario, your test data management gets more complex as you have to create test data that fits all your test
  scenarios.

In our project, we went with the second scenario. We think that it provides a good compromise of runtime requirements
and complexity of test data management. Plus, we think of these tests as integration tests where we do not need to test
...
With the container
images provided by MongoDB and some of the docker magic introduced in this blogpost, it is now possible to run
...
You can find the complete source code for the example described in this blog post in this
